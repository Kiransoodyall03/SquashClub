...
export const deleteTournament = async (tournamentId) => {
  try {
    // Step 1: Get all matches associated with this tournament
    ...
    // Step 2: Delete all matches individually
    if (matchCount > 0) {
      const deletePromises = [];
      
      matchesSnapshot.forEach((matchDoc) => {
        deletePromises.push(deleteDoc(matchDoc.ref));
      });
      
      // Wait for all match deletions to complete
      await Promise.all(deletePromises);
    }
    
    // Step 3: Delete the tournament document
    const tournamentRef = doc(db, 'tournaments', tournamentId);
